...
# DATA
ca_dta <- read.csv("~/Library/CloudStorage/Box-Box/Administrative_Data/Data Dashboard/Clean_20230410.csv")
...
dta <- dta %>% 
  mutate(range = case_when(
    ...
    month == "June" & year == 2022 ~ 12,
    month == "July" & year == 2022 ~ 13,
    month == "August" & year == 2022 ~ 14,
    month == "September" & year == 2022 ~ 15
  ))
...
dta <- dta %>% 
  mutate(date = case_when(
    range == 1 ~ "July 2021",
    range == 2 ~ "August 2021",
    range == 3 ~ "September 2021",
    range == 4 ~ "October 2021",
    range == 5 ~ "November 2021",
    range == 6 ~ "December 2021",
    range == 7 ~ "January 2022",
    range == 8 ~ "February 2022",
    range == 9 ~ "March 2022",
    range == 10 ~ "April 2022",
    range == 11 ~ "May 2022",
    range == 12 ~ "June 2022",
    range == 13 ~ "July 2022",
    range == 14 ~ "August 2022",
    range == 15 ~ "September 2022"
  ))

# UI
ui <- dashboardPage(skin = "black",
                    
                    dashboardHeader(title = "CalWORKS OAR"),
                    ...
                    dashboardSidebar(
                      ...
                      sidebarMenu(
                        ...
                        menuItem("County Comparisons", tabName = "graph"),
                        menuItem("Associations", tabName = "association"),
                            ...
                            tabPanel(
                              ...
                              fluidRow(
                                ...
                                column(
                                    ...
                                    selectizeInput(inputId = "county.varGraph",
                                                   label = "Select County",
                                                   choices = c("Alameda" = "Alameda",
                                                               "Alpine" = "Alpine",
                                                               "Amador" = "Amador",
                                ...
                                column(
                                  ...
                                  selectizeInput(inputId = "var1.varGraph",
                                                 label = "Select Variable",
                                                 choices = c("Sanctions - Overall" = "overall.sanctions",
                                                             "Sanctions - Two Parent" = "two.parent.sanctions",
                                                             "Sanctions - Single Parent with Child Less than Two Years Old" = "single.parent.0024.sanctions",
                                ...
                                column(
                                  ...
                                  selectizeInput(inputId = "var2.varGraph",
                                                 label = "Select Variable",
                                                 choices = c("Sanctions - Overall" = "overall.sanctions",
                                                             "Sanctions - Two Parent" = "two.parent.sanctions",
                                                             "Sanctions - Single Parent with Child Less than Two Years Old" = "single.parent.0024.sanctions",
                                ...
                                column(
                                  ...
                                  actionButton("button7", "Apply"),
                                  width = 3),
                                
                                column(
                                  
                                  plotOutput("varGraph", height = 600),
                                  width = 12,
                                  align = "center")
                            )
                          ),
                            ...
                            tabPanel(
                              ...
                              fluidRow(
                                ...
                                column(
                                  ...
                                  selectizeInput(inputId = "county.cor",
                                                 label = "Select County",
                                                 choices = c("Alameda" = "Alameda",
                                                             "Alpine" = "Alpine",
                                                             "Amador" = "Amador",
                                ...
                                selectizeInput(inputId = "var1.cor",
                                               label = "Select Variable",
                                               choices = c("Sanctions - Overall" = "overall.sanctions",
                                                           "Sanctions - Two Parent" = "two.parent.sanctions",
                                                           "Sanctions - Single Parent with Child Less than Two Years Old" = "single.parent.0024.sanctions",
                              ...
                              column(
                                ...
                                selectizeInput(inputId = "var2.cor",
                                               label = "Select Variable",
                                               choices = c("Sanctions - Overall" = "overall.sanctions",
                                                           "Sanctions - Two Parent" = "two.parent.sanctions",
                                                           "Sanctions - Single Parent with Child Less than Two Years Old" = "single.parent.0024.sanctions",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$button0, {
    ...
    req(input$var, input$var2, input$date)
    
    selectedData_map <- reactive({
      
      dta_map <- dta[, c(input$var, input$var2, 'date', 'county', 'geometry')] %>% filter(date == input$date) %>% na.omit()
    ...
    output$map <- renderLeaflet({
      ...
      pal1 <- colorNumeric(palette = "Blues", domain = dta_map[, input$var])
      pal2 <- colorNumeric(palette = "Oranges", domain = dta_map[, input$var2])
      ...
      leaflet(sf.dta2, options = leafletOptions(zoomControl = FALSE)) %>%
        ...
        addTiles() %>% 
        addPolygons(group = input$var,
                    fillColor = ~pal1(dta_map[, input$var]),
                    fillOpacity = 0.8,
                    color = "#BDBDBD",
                    weight = 1) %>% 
        addPolygons(group = input$var2,
                    fillColor = ~pal2(dta_map[, input$var2]),
                    fillOpacity = 0.8,
                    color = "#BDBDBD",
                    weight = 1) %>%
        addLayersControl(position = "bottomright",
                         baseGroups = c(input$var, input$var2),
  ...
  observeEvent(input$button7, {
    ...
    req(input$county.varGraph, input$var1.varGraph, input$var2.varGraph)
  
  output$varGraph <- renderPlot({
    
    selectedData_varGraph <- reactive({
      
      dta_varGraph <- dta %>% filter(county == input$county.varGraph)
      
    })
    
    dta_varGraph <- selectedData_varGraph()
    
    ggplot(data = dta_varGraph,
           aes(x = range, group = input$county.cor)) +
      geom_line(aes_string(y = input$var1.varGraph)) +
      geom_line(aes_string(y = input$var2.varGraph)) +
      theme_minimal()
    })
  })
  ...
  observeEvent(input$button8, {
    ...
    req(input$county.cor, input$var1.cor, input$var2.cor)
  
  output$corPlot <- renderPlot({
    
    selectedData_corPlot <- reactive({
      
      dta_corPlot <- dta %>% filter(county == input$county.cor) %>% select(input$var1.cor, input$var2.cor)
      
    })
    
    dta_corPlot <- selectedData_corPlot()

    chart.Correlation(dta_corPlot, histogram = TRUE, method = "pearson")
